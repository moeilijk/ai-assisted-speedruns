...
What the game gets, and nothing else. From portal-agent, unchanged: `portal/spt.dll` (SourcePauseTool built with their patch), `portal/addons/spt.vdf`, `portal/cfg/agent_run.cfg`, their `autoexec.cfg` as `portal/cfg/portal_agent.cfg`, and the line `exec portal_agent` in `portal/cfg/autoexec.cfg` (a file already there is backed up once as `.aas-backup`). That configuration is theirs and sets `sv_cheats 1`, `sv_accelerate 100`, `sv_friction 100`, `sv_stopspeed 200` and `fps_max 66.666667`; their [how-it-works](https://github.com/cozyblaze/portal-agent/blob/main/docs/how-it-works.md) says the movement values reduce sliding so the controls are more accurate. It is part of the original tooling and is not changed here. From the harness: `steam_appid.txt` with `400` (what `Portal.bat` writes too), the window size and position on the command line, `+snd_mute_losefocus 0`, and `aas-audio-defaults.json` in the game folder when `AAS_QUIET_AUDIO_DEVICE` is set.
...
Before the run: Portal running with `exec portal_agent` done in its console; OBS running (the recorder builds its scene collection); LiveSplit running (its server starts automatically, see 1b). The agent gets portal-agent's original goal as its first prompt (`goalPrompt` in the plugin; `aas configure --prompt` overrides it). Every test chamber after the first is also an end of its own (`--goal chamber01` … `chamber19`, `chambere00` … `chambere02`, reached when the agent enters that chamber; each has its splits file in [splits/](splits/)): a shorter goal, for tests and first runs, and its prompt names that chamber in the original's words ("… and reach test chamber 05."). The credits stay the default goal. Leave out `--timer livesplit` if LiveSplit is not running; use `--runtime codex` for Codex. `--instructions <file>` and `--goal` override the defaults.
...
- **Scripted player** ([bot.mjs](bot.mjs), runtime `scripted`): a mock run, the end-to-end test of the chain (TAS playback, IGT, chamber milestones, recording, publish) without a model and without tokens. It is not a route: it looks around and walks for `AAS_BOT_PORTAL_STEPS` playbacks (default 8). In the GUI: Run → Portal → Mock run.
- **Displays** (optional): start the game on a secondary display and put LiveSplit next to it with `AAS_PORTAL_WINDOW_POS=X,Y`, `AAS_PORTAL_RESOLUTION=1920x1080` and `AAS_LIVESPLIT_POS=X,Y` in `.env` (coordinates of that display on the Windows desktop), then `npm run portal:launch` and `npm run livesplit:launch -- games/portal/splits/portal-credits.lss`. WGC captures the window there even when it has no focus. Without these the game opens where Windows puts it.
