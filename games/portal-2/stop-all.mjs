#!/usr/bin/env node
// End of a work session: close the programs the harness started (Portal 2, LiveSplit, OBS; Steam when a launcher
// of this harness started it, or always with --steam) the way a user would. `aas run` and `aas resume` do the same
// by themselves when a run ends; this is the manual command for everything else.
