#!/usr/bin/env node
// Close what the harness started for Slay the Spire (game, LiveSplit, OBS; Steam when a launcher of this
// harness started it, or always with --steam), gracefully. `aas run` and `aas resume` do the same by
// themselves when a run ends; this is the manual command for everything else.
