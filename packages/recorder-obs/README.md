...
**The picture is checked, not assumed.** Right after `StartRecord` the recorder asks OBS for its own rendering of the game window source (`GetSourceScreenshot`, 320x180 PNG, decoded to pixels by [png-luma.mjs](png-luma.mjs)) every two seconds. Black for ten seconds: the capture is rebound once and checked for ten seconds more; still black: `start` throws, and `aas run` stops and discards the recording, logs `run.error` and closes everything. A window capture can bind to a window that is gone and then deliver a frozen or empty picture while the game plays on, and OBS's log says nothing about it. At publish time `aas publish` measures black intervals in every raw recording with ffmpeg and `aas check` reports them.
