...
**A running screensaver is ended before the recording starts.** Windows starts it after its idle time, on a desktop of its own, and while it runs the window capture of the game shows black (measured 2026-09-26: the game rendered, the capture gave black for 20 s, the run did not start). The recorder ends it the way Windows documents, by closing the windows on the `Screen-saver` desktop (`packages/core/src/windows/screensaver.mjs`), and says so in the log; `keep-display-awake` does the same when it starts, since `ES_DISPLAY_REQUIRED` only keeps a screensaver from starting.
