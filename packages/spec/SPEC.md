...
instead of a model, to test a machine without spending tokens (a mock run). Such a bundle may be complete and
signed, and it is still not a run of an AI: an archive does not publish it, rank it or keep it as an entry.
...
two things instead, none of which is the publisher's to write afterwards:
...
Missing, unreadable or contradictory evidence makes a bundle a mock; only both together make it a run of an
...
the bundle carries the hash of both (§8.9). A publisher who writes the route into the prompt publishes that route,
in the bundle, for every reader to see. Dictating is then a visible choice about the run, which an
...
| `proof.json` | when the run was recorded with proof | the tickets, the heads and the archive's receipts that fix the logs while the run went on, see §8.11 |
...
A run recorded with proof (§8.11) adds a **private part** under `<run_id>/private/`: the logs the proof covers, as they are in the run directory, so the archive can check them. `run.jsonl`, the runtime's own session logs as `session-<n>.jsonl` in the order `proof.json` numbers them, `recording.json` and `brief.json`. They hold what the public timeline holds and what the sanitiser takes out of it, such as the publisher's local paths and user name. The private part is not in `manifest.json` and not covered by `signature.json`; it is bound to the bundle only through `proof.json`. An archive uses it for the check at submission and for nothing else: it never publishes it, never puts it in a downloadable zip and never shows it, and deletes it as soon as the submission is decided, or after 30 days when a reviewer has not decided by then. In a later dispute the archive asks the publisher to upload it again, and it must match the (length, sha256) pairs in `proof.json`.
...
9. The bundle says what the model was told. `AGENTS.md` is the instructions verbatim and `category.goal_prompt` the first prompt verbatim; `brief.instructions_sha256` and `brief.goal_prompt_sha256` are their sha256 and are recomputed from the bundle. A bundle whose hashes differ from its own `AGENTS.md` or goal prompt fails this point.
...
11. The logs were fixed while the run went on. Before a segment is recorded the harness fetches a ticket from the archive, signed by the archive (`aas-ticket v1`, `ticket: <32 hex>`, `issued: <time>`). At the start of the segment, every hour during it and at its end, it takes the run log and the runtime's session logs as they stand and sends the archive a head: the sha256 of the text `aas-head-input v1`, `prev: <the previous head, or the first ticket>`, and per file, sorted by label, `<label> <length in bytes> <sha256 of those bytes>`, joined with newlines. Labels are `run` and `session:<n>`. Only the ticket and the head go over the line. The archive answers with a receipt over `aas-head v1`, `ticket`, `seq`, `kind` (`start`, `hour`, `end`), `segment`, `head` and `received: <its own time>`, keeps what it received, and refuses a second head for the same `seq`. `proof.json` carries the tickets without their control secret, every head with the files it covered, every receipt, the heads the archive did not acknowledge (`missed`) and which session log the timeline was made from. The archive's keys are published at `/.well-known/aas-proof.txt`.

    At submission the archive checks every signature, recomputes every head from the private part (§7a) on the prefix of each file, compares them with the heads it received itself, and makes `session.sanitized.jsonl` and `timeline.json` again from the private part with the same function the harness used; they must come out byte for byte the same. A broken chain, a signature that is not the archive's, or a head or timeline that does not match is tampering and the submission is refused. A segment without a received head, a head that never reached the archive, an account that is not the ticket's, a ticket that was submitted before, or two heads for one `seq` go to a reviewer. A run recorded without proof is accepted and marked unsigned.
...
| 0.41 | 2026-09-23 | §8.9: the harness sends nothing to an archive by itself; the archive no longer witnesses segments (owner, 2026-09-23). `run.witnessed` and `run.unwitnessed` are no longer reserved and are not checked. §3: whether a model played follows from the runtime's own hash and the timeline's evidence. §8.10: the prompt hashes are checked against the bundle only. Schema 16 unchanged |
