...
test("the sheet offers the cut and the full recording per segment, each with its own line, length and chapters", () => {
  ...
  assert.equal(cutText[0], 'Claude Sonnet 5 played Slay the Spire in Claude Code and reached the goal "Act 1 boss" in 2m 57.4s in-game time (22m 42.6s real time).', "the result first");
  assert.equal(cutText[1], "AAS Archive: ai-assisted-speedruns.org — run sts-claude-code-01", "the domain and the run id as text, no link YouTube would shorten");
  ...
  assert.ok(part1.includes("Title: Slay the Spire · Act 1 boss in 2m 57.4s · Claude Sonnet 5 · part 1 of 2"));
  assert.ok(part2.includes(`line:  ${fp} · 191 s`));
  assert.ok(part2.includes("0:04 A human restarted the agent after it reached its goal"), "part 2's chapters start from its own 0:00, without the harness's details");
