...
import { buildTimeline, mergeHarnessEvents, readRunEvents } from "./timeline-build.mjs";
export { buildTimeline, mergeHarnessEvents };
...
export async function publish(runDir, outDir, { session, completionMarker, log = console.log , signKey = null } = {}) {
  ...
  const exportSession = rt?.exportSession ? (input, dest, o) => rt.exportSession(input, dest, o) : null;
  const { history, goals, won, publishedGoal, timeline } = await buildTimeline({ runDir, outDir, brief, runtimeId: runtime, exportSession, session, completionMarker, log });
