...
- `npm run e2e:real` puts every game's mock through the real GUI on the real programs (Playwright): sound off the speakers, bundle, upload, nothing left running, settings unchanged. Run it after a change to a game, the recorder, the timer or the GUI.
