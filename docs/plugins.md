...
- The tests run against a stand-in for the game's endpoint, the way `games/*/test/fake-*.mjs` do, so `npm test` needs no game installed. Add the plugin's files to `check` in `package.json`.
