...
1. `aas publish <run-dir> <out-dir>` writes the bundle and the upload sheet, and prints the line
   `AAS <run-id> · fingerprint <16 hex> · <n> s`, where `n` is the length of the video it goes with.
2. Upload the video from the private run directory, not from the public folder: `<run>/recording/UPLOAD.txt`
   ...
   line its description needs, carrying that video's length; the title and description in it are an example. Put the line in the field a viewer can read on that platform: the description, or
...
| `run.witnessed`, `run.unwitnessed` | harness (run, resume) | the archive's receipt for the start or end of a segment, with the signed statement; or why there is none (no network, no publisher key, the witness refused) |
