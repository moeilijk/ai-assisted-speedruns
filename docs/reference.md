...
single line to register with an archive. Where a code host publishes an account's keys (GitHub at
`https://github.com/<user>.keys`) an archive can match it without that registration step, but that is a
convenience for people who have such an account, not the mechanism. It is not an upload credential: the upload
is the zip through a submission form. The signature says that the same key published these
bundles; whose key it is, is for an archive to record. An unsigned bundle is still readable and checkable — a
fixture, a local copy — but `aas check` reports its signature as unmet, because an entry says who published it.
A requirement to sign only means something together with the account matching: anyone can make a key and sign
anything. Setting a key up:
