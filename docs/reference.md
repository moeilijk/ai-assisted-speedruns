...
| | `aas upload-sheet` | `recording/UPLOAD.txt` in the run directory: the video file, the fingerprint line it needs, and an example title and description |
...
| `aas upload-sheet <run-dir> [--bundle <public-dir>] [--note <text>]` | Writes `<run>/recording/UPLOAD.txt`, next to the videos in the private run directory, with only what uploading the video to a video site takes: the file (the `.cut.mp4` when `aas render` made one, else the recording), the one requirement, the fingerprint line the description has to contain (the title where a site has no description), and as an example, not a requirement, a title and a description: the run's result, the chapters in YouTube's format (labels without the harness's details: save name, exit code, turns, cost), the run's page in the archive, and the fingerprint line at the end. The figures come from the published bundle's `summary.json`, the chapters from `timeline/chapters.cut.txt` (or `chapters.txt` without a cut). `aas publish` writes it and remembers the bundle directory in `publish-revision.json`; `aas render` writes it again for a published run. `--note` adds a sentence to the description and is remembered for later sheets. |
...
2. Upload the video from the private run directory, not from the public folder: `<run>/recording/UPLOAD.txt`
   names the file (the cut `aas render` made, or the recording) and the line; the title and description in it are
   an example. Put the line in the field a viewer can read on that platform: the description, or
