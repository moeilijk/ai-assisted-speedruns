...
`--sign [key]` signs the bundle with an ed25519 key — without a path, `~/.ssh/id_ed25519`, the key the
publisher already pushes with, so the common case needs no new key and no registration: the
