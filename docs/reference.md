...
| `AAS_ARCHIVE_FETCH` | every command that talks to the Archive, and the GUI | A module whose `archiveFetch` authenticates each request to the Archive itself, next to the token of `aas login`; `aas login` then follows the Archive's answer itself instead of opening a browser. Off by default. |
