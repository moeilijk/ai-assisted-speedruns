...
| | `aas upload-sheet` | `recording/UPLOAD.txt` in the run directory: the video to upload, the fingerprint line, title, description and chapters |
...
| `aas upload-sheet <run-dir> [--bundle <public-dir>] [--note <text>]` | Writes `<run>/recording/UPLOAD.txt`, next to the videos in the private run directory: where the files are (the video is in the run directory, not in the public folder), the file to upload (the `.cut.mp4` when `aas render` made one, else the recording), the fingerprint line, a title, a description to paste as it stands, with the run's result first and the fingerprint line at the end, the chapters in YouTube's format (labels without the harness's details: save name, exit code, turns, cost) with whether YouTube will show them as chapters, and the zip with where to submit it. The figures come from the published bundle's `summary.json`, the chapters from `timeline/chapters.cut.txt` (or `chapters.txt` without a cut). `aas publish` writes it and remembers the bundle directory in `publish-revision.json`; `aas render` writes it again for a published run. `--note` adds a sentence to the description and is remembered for later sheets. |
...
2. Upload the video from the private run directory, not from the public folder: `<run>/recording/UPLOAD.txt`
   names the file (the cut `aas render` made, or the recording) and holds the title, the description with that
   line in it, and the chapters. Put the line in the field a viewer can read on that platform: the description, or
